...
const tabs = [
  ...
  { path: "/insights", label: "Coach", Icon: SparkIcon },
...
export default function BottomNav() {
  ...
  return (
    <nav className="pointer-events-none absolute inset-x-0 bottom-6 z-[70] flex justify-center pb-[env(safe-area-inset-bottom)]">
      <div className="pointer-events-auto flex w-[min(100%,400px)] items-end gap-0.5 rounded-full border border-black/5 bg-[rgba(235,238,242,0.94)] py-2 pl-1 pr-2 shadow-[0_8px_24px_rgba(16,18,23,0.14)] backdrop-blur-md">
        {tabs.map((t) => {
          ...
          return (
            <Link
              key={t.path}
              to={t.path}
              className={`flex min-w-0 flex-1 flex-col items-center justify-center rounded-full px-0.5 py-1.5 ${
                active ? "bg-white/80 text-brand-accent" : "text-brand-text-muted"
              }`}
            >
              <t.Icon active={active} />
              <span className="mt-0.5 max-w-full truncate font-mono text-[9px] font-semibold leading-tight">
                {t.label}
              </span>
            </Link>
          );
